...
class DoubleRing {
	...
	private class Node {
		...
		public char val(char c) { return m_val; }
	}

//// Class member variables

	
	private Node m_current; // Node that tracks the 'current' i.e. currently-selected Node in this ring.

//// Class methods
	DoubleRing () { m_current = null; }

	char current() {// Return current node's value
		if (m_current == null) {
			return (char) 0;
		}
	...
	void add(Node n){
		...
		n.next = m_current.next;// Leave m_current pointing to the same node as before the call to add()
		n.prev = m_current;
		m_current.next = n;
		n.next.prev = n;
		}
 // Insert 'n' into this ring, after current and before current.next 
	...
	void add(char c) {// Helper function so we can add chars rather than Nodes to this ring.
		add(new Node(c));
	}
	...
	void remove() throws IndexOutOfBoundsException{
        ...
        m_current = m_current.prev;     // Adjust the references
	...
	void PrintRing() {
		final DoubleRing.Node temp = m_current;
        do {
            System.out.print(m_current + " ");// should just print out the value of the current 
            m_current = m_current.next;
        } while (m_current!= temp);
        System.out.println();
    }
