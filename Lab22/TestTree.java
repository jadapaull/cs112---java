...
class Tree {
	 ...
	 */
	public int min(TreeNode root) throws Exception {
		if (root == null) { 
...
public class TestTree {
	public static void main(String[] args) throws Exception {
		...
		String filename = args[0];// get filename from args[0]
		File file = new File(filename);
		Scanner scanner = new Scanner(file);

		while (scanner.hasNextInt()) {
			int number = scanner.nextInt();	// READ INTS FROM A FILE WHOSE
			myTree.add(number);// add numbers to tree

		System.out.println("Sum is " + myTree.sum());
		System.out.println("Min is " + myTree.min(null));
